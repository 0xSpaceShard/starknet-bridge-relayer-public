...
library NamedStorage {
    ...
    function uintToAddressMapping(
        string memory tag_
    ) internal pure returns (mapping(uint256 => address) storage randomVariable) {
        bytes32 location = keccak256(abi.encodePacked(tag_));
        assembly {
            randomVariable_slot := location
        }
...
library CommitmentTreeUpdateOutput {
    ...
    */
    function getPrevRoot(uint256[] calldata commitmentTreeUpdateData) internal pure returns (uint256) {
    ...
    */
    function getNewRoot(uint256[] calldata commitmentTreeUpdateData) internal pure returns (uint256) {
...
library StarknetOutput {
    ...
    */
    function getMerkleUpdate(uint256[] calldata output_data) internal pure returns (uint256[] calldata) {
    ...
    */
    function processMessages(
        ...
        uint256 payloadSizeOffset = (isL2ToL1 ? MESSAGE_TO_L1_PAYLOAD_SIZE_OFFSET : MESSAGE_TO_L2_PAYLOAD_SIZE_OFFSET);
...
contract StarknetMessaging is IStarknetMessaging {
    ...
    */
    function getL1ToL2MsgHash(
        ...
        return keccak256(abi.encodePacked(uint256(msg.sender), toAddress, nonce, selector, payload.length, payload));
    }
    ...
    */
    function consumeMessageFromL2(uint256 fromAddress, uint256[] calldata payload) external override returns (bytes32) {
        bytes32 msgHash = keccak256(abi.encodePacked(fromAddress, uint256(msg.sender), payload.length, payload));
    ...
    ProxySupport
{
    ...
    //     (uint256 programHash_, address verifier_, uint256 configHash_, StarknetState.State memory initialState) = abi
    //         .decode(data, (uint256, address, uint256, StarknetState.State));
    ...
    */
    function updateState(
        ...
        require(configHash() == programOutput[StarknetOutput.CONFIG_HASH_OFFSET], 'INVALID_CONFIG_HASH');
